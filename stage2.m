...
%            tocompare = 1 & 0 matrix=> 1 = known number
%                                     2 = unknown number
...
function A = stage2(M, M_shadow, step_size)
    ...
    % Then, A = prev_result;
    while (round(sum(diag(S)),5)< nuclear_norm && real_loop_time<300 )
        nuclear_norm = round(sum(diag(S)),5); %update nuclear norm
       disp(['nuclear norm at iteration: ',num2str(real_loop_time),' is ', num2str(nuclear_norm,20)]);
